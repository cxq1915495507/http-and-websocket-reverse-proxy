...
type Session struct {
	...
	ctrlStream *Stream			//ctrl stream

	HandleCtrlMsg func(msg *CtrlMsg)
	HandleCtrlErr func()
...
func newSession(config *Config, conn io.ReadWriteCloser, client bool) *Session {
	...
	if !client {
		s.nextStreamID = 1
		s.ctrlStream, err = s.OpenCtrlStream()
		if err != nil{
			log.Fatal("s.OpenCtrlStream()")
			return nil
		}
	} else {
		s.nextStreamID = 0
		s.ctrlStream, err = s.AcceptStream()
		if err != nil || s.ctrlStream.id != ctrlStreamId {
			log.Fatal("s.OpenCtrlStream()")
			return nil
		}
	}
	go listenCtrlStream(s.ctrlStream)
	return s
}
...
// OpenStream is used to create a new stream
func (s *Session) OpenCtrlStream() (*Stream, error) {
	if s.IsClosed() {
		return nil, errors.New(errBrokenPipe)
	}
	sid := uint32(ctrlStreamId)
	stream := newStream(sid, s.config.MaxFrameSize, s)
...
func (s *Session) WriteCtrlStream(buf [] byte){
	s.ctrlStream.Write(buf)
}
