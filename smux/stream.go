...
func listenCtrlStream(conn io.ReadWriteCloser){
	reader := bufio.NewReader(conn)
	for{
		line, err := reader.ReadString('\n')
		if err != nil {
			log.Println("reader.ReadString", err)
			if conn.(*Stream).sess.HandleCtrlErr != nil {
				conn.(*Stream).sess.HandleCtrlErr()
			}
			return
		}
		if ctrlMsg := DecodeCtrlMsg(line); ctrlMsg != nil{
			if conn.(*Stream).sess.HandleCtrlMsg != nil {
				conn.(*Stream).sess.HandleCtrlMsg(ctrlMsg)
			}
		}else{
			log.Println(" smux.DecodeCtrlMsg(line)")
		}
	}
}

func EncodeCtrlMsg(cmd string, data string) []byte {
	mess := CtrlMsg{cmd, data}
...
func DecodeCtrlMsg(str string) *CtrlMsg{
	var mess = new(CtrlMsg)
